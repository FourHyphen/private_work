...
- 異常を検知した場合
  - 本プロセスは以下を実行する
    - `process.send({ type: 'エラー種別', kind: 終了コード, reason: 'エラー内容' });` を実行してメッセージ通知
      - 終了コード未設定時は `99` を送信する
    - `process.send()` の完了を待機してから `process.exit(終了コード、非0)` を実行する
...
- 書き込み権限不足などの場合
- この場合のメインプロセスとの契約
  - `データ保存ファイル保存先ディレクトリ作成失敗` と同じとする

### データ保存ファイルへの書き込み: ハング
- 内部的には書き込み中フラグの true / false によりポーリング時に書き込み処理を実行するかを決定する
- 書き込み処理がハングするとフラグが false に戻らないため以降書き込み処理を実行しなくなる
- この場合の対策やメインプロセスとの契約
  - `save-write-hang` イベントを `process.send()` で通知する
  - 終了コード: `4`
  - 親プロセスは `exit` / `message` / タイムアウトを監視して再起動処理を行う
