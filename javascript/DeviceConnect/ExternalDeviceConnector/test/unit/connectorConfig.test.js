...
describe('ConnectorConfig: 設定値を検証する', () => {
  it.each([
    ['数値', 123],
    ['null', null]
  ])('deviceUrl が%sの場合は TypeError(kind: 1) を送出する', (_, deviceUrl) => {
    expect(() => new ConnectorConfig({ ...VALID_WITHOUT_DATAFILEPATH, deviceUrl }))
  ...
  it.each([
    ...
    ['小数', 3.5],
    ['文字列', '4000']
  ])('mainPort が%sの場合は TypeError(kind: 1) を送出する', (_, mainPort) => {
    expect(() => new ConnectorConfig({ ...VALID_WITHOUT_DATAFILEPATH, mainPort }))
      .toThrow(expect.objectContaining({ kind: 1 }));
  });
  ...
  it('saveFile が空文字列の場合は TypeError(kind: 1) を送出する', () => {
    expect(() => new ConnectorConfig({ ...VALID_WITHOUT_DATAFILEPATH, saveFile: '' }))
      .toThrow(expect.objectContaining({ kind: 1 }));
  });
  ...
  it.each([
    ...
    ['負の数', -1],
    ['文字列', '1000']
  ])('saveFile が定義されていてかつ rotationKb が%sな場合は TypeError(kind: 1) を送出する', (_, rotationKb) => {
    expect(() => new ConnectorConfig({ ...VALID_WITH_DATAFILEPATH, saveFile: { ...VALID_WITH_DATAFILEPATH.saveFile, rotationKb } }))
      .toThrow(expect.objectContaining({ kind: 1 }));
  });
  ...
  it.each([
    ...
    ['負の数', -1],
    ['文字列', '1000']
  ])('dataFilePath が定義されていてかつ maxSaveFileNum が%sな場合は TypeError(kind: 1) を送出する', (_, maxSaveFileNum) => {
    expect(() => new ConnectorConfig({ ...VALID_WITH_DATAFILEPATH, saveFile: { ...VALID_WITH_DATAFILEPATH.saveFile, maxSaveFileNum } }))
      .toThrow(expect.objectContaining({ kind: 1 }));
  });
});
...
describe('ConnectorConfig: JSON 引数から設定を生成する', () => {
  ...
  it('JSON 引数がない場合は Error(kind: 1) を送出する', () => {
    // argv[2] が存在することを正とする
    expect(() => ConnectorConfig.fromArgv(['node', 'main.js'])).toThrow(expect.objectContaining({ kind: 1 }));
    expect(() => ConnectorConfig.fromArgv(['node', 'main.js'])).toThrow(Error);
  });

  it('argv が配列でない場合は Error(kind: 1) を送出する', () => {
    // argv が配列であることを正とする
    expect(() => ConnectorConfig.fromArgv(null)).toThrow(expect.objectContaining({ kind: 1 }));
    expect(() => ConnectorConfig.fromArgv(null)).toThrow(Error);
  });

  it('JSON として不正な場合は Error(kind: 1) を送出する', () => {
    expect(() => ConnectorConfig.fromArgv(['node', 'main.js', 'not-json'])).toThrow(expect.objectContaining({ kind: 1 }));
  ...
  it.each([
    ...
    ['null', 'null'],
    ['配列', '[1,2,3]']
  ])('JSON が%sの場合は TypeError(kind: 1) を送出する', (_, json) => {
    expect(() => ConnectorConfig.fromArgv(['node', 'main.js', json])).toThrow(expect.objectContaining({ kind: 1 }));
  });
});
